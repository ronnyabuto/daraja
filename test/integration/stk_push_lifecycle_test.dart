...
const _testPhone = String.fromEnvironment('TEST_PHONE', defaultValue: '0712345678');
...
void main() {
  ...
  group('Full STK Push lifecycle', () {
    test('happy path — PaymentSuccess received after PIN entry', () async {
      ...
      stream.listen((state) {
        states.add(state);
        if (state is PaymentSuccess || state is PaymentFailed ||
            state is PaymentCancelled || state is PaymentTimeout ||
      ...
      expect(states, containsAll([isA<PaymentInitiating>(), isA<PaymentPending>()]));
    ...
    test('user cancels — PaymentCancelled received', () async {
      ...
      await for (final state in stream) {
        states.add(state);
        if (state is PaymentCancelled || state is PaymentSuccess ||
            state is PaymentFailed || state is PaymentTimeout) break;
      }
    ...
    test('insufficient funds — PaymentFailed with correct resultCode', () async {
      ...
      await for (final state in stream) {
        states.add(state);
        if (state is PaymentFailed || state is PaymentSuccess ||
            state is PaymentCancelled || state is PaymentTimeout) break;
      }
    ...
    test('T+90s hard timeout — PaymentTimeout emitted, not PaymentFailed', () async {
      ...
      await for (final state in stream) {
        states.add(state);
        if (state is PaymentTimeout || state is PaymentSuccess ||
            state is PaymentFailed || state is PaymentCancelled) break;
      }

      expect(states.last, isA<PaymentTimeout>(),
          reason: 'Timeout must be distinct from failure — '
              'money may have been deducted');
    }, timeout: const Timeout(Duration(seconds: 100)));

    test('duplicate callback — only one terminal state emitted', () async {
    ...
    test('killed-app recovery — pending payment restored and resolved on restart',
        () async {
...
extension on Daraja {
  Stream<PaymentState> get stream => (this as dynamic)._notifier.stream as Stream<PaymentState>;
}
