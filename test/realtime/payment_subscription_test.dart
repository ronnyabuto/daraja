...
void main() {
  ...
  group('poll()', () {
    ...
    test('returns PaymentSuccess with correct fields when status is SUCCESS',
        () async {
    ...
    test('returns PaymentFailed with resultCode and message when status is FAILED',
        () async {
      stubGetDocument(
    ...
    test('returns null when status is null — document exists but not yet resolved',
        () async {
    ...
    test('returns null on 404 — document not yet written by the function',
        () async {
      when(() => mockDatabases.getDocument(
            databaseId: any(named: 'databaseId'),
            collectionId: any(named: 'collectionId'),
  ...
  group('stream', () {
    ...
    test('emits PaymentTimeout on TIMEOUT event', () async {
      realtimeController.add(
        realtimeMessage(status: 'TIMEOUT', cid: testCid),
      );
    ...
    test('ignores events that are not create or update', () async {
      ...
      realtimeController.add(RealtimeMessage(
        events: [
